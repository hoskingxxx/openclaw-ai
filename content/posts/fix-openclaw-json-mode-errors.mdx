...
I hit this the very first time I connected OpenClaw to Ollama running DeepSeek R1. The console exploded with `Unexpected token < in JSON at position 0`. As soon as I stripped the thinking tags via regex, the exact same request parsed perfectly. This confirms the model isn't "broken", it's just too chatty.
...
*Context: This proves exactly why `JSON.parse` fails at "line 1 column 1" (because `<` is not `{`).*
...
| Tool Calling | DeepSeek is configured for function calling | Disable thinking tags in config |
...
> For a full breakdown of correct R1 parameters, check our **[DeepSeek R1 Configuration Guide](/posts/how-to-use-deepseek-with-openclaw)**.
...
2. **For Production Stability:**
   ...
   Parsing is just one hurdle. If your hardware is also struggling, see our **[CUDA OOM Fix Guide](/posts/fix-openclaw-cuda-oom-errors)**.
...
- [How to Fix OpenClaw OOM Errors](/posts/fix-openclaw-cuda-oom-errors) - VRAM optimization tips

- **DeepSeek R1 Optimization Guide** *(Coming Soon)* - Context window and performance tuning
